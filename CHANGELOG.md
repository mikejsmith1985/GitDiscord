...
### Added
- **Pin-ready `/help-public` slash command** (`src/bot/commands/link_commands.py`, `tests/test_main.py`, `README.md`): Added a public help command that posts a non-ephemeral command guide message suitable for pinning in Discord channels.
...
### Fixed
- **Noisy NLP unknown-message reactions** (`src/nlp/command_parser.py`, `tests/test_main.py`): NLP mode now silently ignores unrecognized messages (and unlinked NLP-channel chatter) instead of reacting with `❓` on every non-command post.
